...
import static de.walware.ecommons.waltable.viewport.ViewportLayer.PAGE_INTERSECTION_SIZE;
...
public class ViewportLayerDim extends TransformLayerDim<ViewportLayer> implements IViewportDim {
	...
	public ViewportLayerDim(/*@NonNull*/ final ViewportLayer layer, /*@NonNull*/ final ILayerDim underlyingDim) {
	...
	protected void recalculateAvailableSizeAndPositionCount() {
		...
		this.cachedOriginPosition= (getMinimumOriginPosition() < positionBound) ?
				this.underlyingDim.getPositionByPixel(getOriginPixel()) :
				0;
		this.cachedPositionCount= 0;
		long position= this.cachedOriginPosition;
		if (position >= 0 && position < positionBound) {
			this.cachedOriginPositionPixelShift= (int) (getOriginPixel() - this.underlyingDim.getPositionStart(position));
	...
	public void setMinimumOriginPosition(final long scrollablePosition) {
		if (scrollablePosition < 0 || scrollablePosition > this.underlyingDim.getPositionCount()) {
			throw new PositionOutOfBoundsException("scrollablePosition: " + scrollablePosition); //$NON-NLS-1$
		}
	...
	/**
	 * LRange checking for origin pixel position.
	 * @param pixel
	...
	private long boundsCheckOrigin(final long pixel) {
		...
		final long max= Math.max(this.underlyingDim.getSize() - 1, min);
		if (pixel >= max) {
			return max;
		}
		return pixel;
	...
	public void movePositionIntoViewport(final long scrollablePosition) {
		if (this.underlyingDim.getPositionId(scrollablePosition, scrollablePosition) >= 0
			...
			if (scrollablePosition <= getOriginPosition()) {
				...
				// Move forward
				final long scrollableStart= this.underlyingDim.getPositionStart(scrollablePosition);
				final long scrollableEnd= scrollableStart + this.underlyingDim.getPositionSize(scrollablePosition);
				final long clientAreaSize= getClientAreaSize();
				final long originPosition= getOriginPosition();
				final long viewportEnd= this.underlyingDim.getPositionStart(originPosition) + clientAreaSize;
...
//		}
	...
	public void scrollForwardByStep() {
		...
		if (position + 1 < this.underlyingDim.getPositionCount()) {
			final long sizeForPosition= this.underlyingDim.getPositionStart(position + 1) - origin;
			if (sizeForPosition > 0 && sizeForPosition <= stepSize) {
				setOriginPixel(origin + sizeForPosition);
				return;
			}
...
//		}
	...
	public void scrollBackwardByPosition() {
		...
		final long position= getOriginPosition();
		final long positionPixel= getPositionStart(position);
	...
	public void scrollBackwardByPage() {
		...
		for (final long bound= getMinimumOriginPosition(); position > bound; ) {
			final long size= this.underlyingDim.getPositionSize(position - 1);
	...
	public void scrollForwardByPage() {
		...
		for (final long bound= this.underlyingDim.getPositionCount(); position + 1 < bound; ) {
			final long size= this.underlyingDim.getPositionSize(position);
			if (sizeForPosition + size <= pageSize) {
				sizeForPosition+= size;
				position++;
				continue;
	...
	protected void handleStructuralChange(final Collection<StructuralDiff> diffs) {
		...
		if (diffs != null) {
			...
			if (selectedOriginPosition >= 0) {
				final long pixel= this.underlyingDim.getPositionStart(selectedOriginPosition + selectedPositionChange);
				if (pixel >= 0) {

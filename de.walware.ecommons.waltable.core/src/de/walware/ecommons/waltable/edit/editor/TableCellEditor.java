...
public class TableCellEditor extends AbstractCellEditor {
	 ...
	 * 			open the cell for editing instead of moving into the selection state of the table control.
	 */
	public TableCellEditor(final int fixedSubCellHeight, final boolean moveSelectionOnEnter, final boolean alwaysOpenEditor) {
		setFixedSubCellHeight(fixedSubCellHeight);
	...
	public Object getCanonicalValue() {
		final Object[] editorValues= getDataAsArray(getEditorValue());
		//perform conversion
		final Object[] dataValues= new Object[editorValues.length];
		Object canonicalValue;
		for (int i= 0; i < editorValues.length; i++) {
			final Object value= editorValues[i];
	...
	public void setCanonicalValue(final Object canonicalValue) {
		final Object[] values= getDataAsArray(canonicalValue);
		if (values != null) {
			//transform the array of canonical data values to an array of display values
			final ValueWrapper[] editorValues= new ValueWrapper[values.length];
			Object displayValue;
	...
	@Override
	public Table createEditorControl(final Composite parent) {
		...
		final TableViewerColumn emptyColumn= new TableViewerColumn(this.viewer, SWT.NONE);
		...
		final TableViewerColumn singleColumn= new TableViewerColumn(this.viewer, SWT.NONE);
		singleColumn.getColumn().setAlignment(CellStyleUtil.getHorizontalAlignmentSWT(this.cellStyle, SWT.LEFT));
		...
		tableControl.addListener(SWT.MeasureItem, new Listener() {
			...
			public void handleEvent(final Event event) {
				event.height= TableCellEditor.this.fixedSubCellHeight + 1; //+1 because of the grid lines
			}
		});
		...
		tableControl.addListener(SWT.Resize, new Listener() {
			...
			public void handleEvent(final Event event) {
				...
				final Point mouseLoc= Display.getCurrent().getCursorLocation();
				final Point tablePos= tableControl.toDisplay(0, 0);
				final int relativeX= mouseLoc.x - tablePos.x;
				final int relativeY= mouseLoc.y - tablePos.y;
				final TableItem item= tableControl.getItem(new Point(relativeX, relativeY));
				if (item != null) {
	 ...
	 */
	public void setFixedSubCellHeight(final int fixedSubCellHeight) {
	...
	protected class ValueWrapper {
		...
		protected ValueWrapper(final Object value) {
			setValue(value);
		}
	...
	protected class InternalLabelProvider extends ColumnLabelProvider {
		...
		private final IStyle normalStyle;
		 ...
		 */
		public IStyle getActiveCellStyle(final Object element) {
		 ...
		 */
		public void applyCellStyle(final Control editorControl, final Object element) {
			...
			final Font font= getFont(element);
			if (font != null) {

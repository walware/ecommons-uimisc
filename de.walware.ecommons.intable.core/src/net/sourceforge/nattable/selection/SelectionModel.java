...
public class SelectionModel implements ISelectionModel {
	...
	private static int[] subarray(int[] array, int beginIndex, int endIndex) {
		if (beginIndex == 0 && array.length == endIndex) {
			return array;
		}
	...
	public int[] getFullySelectedRows(int rowWidth) {
		...
		return subarray(fullySelectedRows, 0, index);
	}
	...
	public int[] getFullySelectedColumns(int fullySelectedColumnRowCount) {
		...
		if (index == columnsToHide.length) {
			return columnsToHide;
		}
		
		return subarray(columnsToHide, 0, index);
	}

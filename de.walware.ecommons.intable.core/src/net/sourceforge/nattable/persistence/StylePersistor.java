...
public class StylePersistor {
	...
	protected static void saveBorder(String prefix, Properties properties, BorderStyle borderStyle) {
		...
		String value = borderStyle.getThickness() + "|" +
				ColorPersistor.asString(borderStyle.getColor()) + "|" +
				borderStyle.getLineStyle();
		properties.setProperty(prefix + DOT + BORDER_PREFIX, value);
	}
	...
	private static BorderStyle loadBorderStyle(String borderPrefix, Properties properties) {
		...
		if (borderStyle != null) {
			final String[] tokens = borderStyle.split("\\|");
			return new BorderStyle(Integer.parseInt(tokens[0]),
					ColorPersistor.asColor(tokens[1]),
					LineStyleEnum.valueOf(tokens[2]) );
		}
		return null;
	}

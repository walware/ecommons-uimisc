...
//					modifiedRows.add(beforeIndex, beforePositionRange.start);
	...
	public static void handleColumnDelete(Collection<StructuralDiff> columnDiffs, ILayer underlyingLayer, 
			Collection<Long> cachedColumnIndexes, boolean handleNotFound) {
		...
		for (Iterator<StructuralDiff> diffIterator = columnDiffs.iterator(); diffIterator.hasNext();) {
			...
			if (columnDiff.getDiffType() != null && columnDiff.getDiffType().equals(DiffTypeEnum.DELETE)) {
				...
				for (long i = beforePositionRange.start; i < beforePositionRange.end; i++) {
					long index = i;//underlyingLayer.getColumnIndexByPosition(i);
					if (index >= 0)
						toRemove.add(index);
					else
						numberOfNoIndex++;
		...
		List<Long> modifiedColumns = new ArrayList<Long>();
		for (Long column : cachedColumnIndexes) {
			//check number of removed indexes that are lower than the current one
			...
			long modColumn = column-deletedBefore;
			if (modColumn >= 0)
	...
	public static void handleColumnInsert(Collection<StructuralDiff> columnDiffs, ILayer underlyingLayer, 
			Collection<Long> cachedColumnIndexes, boolean addToCache) {
		
		for (StructuralDiff columnDiff : columnDiffs) {
			if (columnDiff.getDiffType() != null && columnDiff.getDiffType().equals(DiffTypeEnum.ADD)) {
				Range beforePositionRange = columnDiff.getBeforePositionRange();
				List<Long> modifiedColumns = new ArrayList<Long>();
				long beforeIndex = underlyingLayer.getColumnIndexByPosition(beforePositionRange.start);
				for (Long column : cachedColumnIndexes) {
...
//					modifiedColumns.add(beforeIndex, beforePositionRange.start);
	...
	public static boolean isReorder(Collection<StructuralDiff> structuralDiffs) {
		...
		if (structuralDiffs != null && structuralDiffs.size() == 2) {
			long numberOfDeleteCols = 0;
			long numberOfInsertCols = 0;

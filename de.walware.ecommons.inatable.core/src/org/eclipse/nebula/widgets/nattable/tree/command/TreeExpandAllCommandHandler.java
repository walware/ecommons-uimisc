...
public class TreeExpandAllCommandHandler implements ILayerCommandHandler<TreeExpandAllCommand> {
	...
	public boolean doCommand(ILayer targetLayer, TreeExpandAllCommand command) {
		...
		List<Long> rootIndexes = treeModel.getRootIndexes();
		Collections.reverse(rootIndexes);
		
		for (long root : rootIndexes) {
	 ...
	 * 			state and children.
	 */
	private void expandChild(long index, ITreeRowModel<?> treeModel) {
	 ...
	 * 			state and children.
	 */
	private void expandChilds(List<Long> childIndexes, ITreeRowModel<?> treeModel) {
		Collections.reverse(childIndexes);
		for (Long index : childIndexes) {

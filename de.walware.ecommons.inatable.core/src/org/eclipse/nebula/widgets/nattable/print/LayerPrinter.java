...
public class LayerPrinter {
	...
	private Point getPageCount(Printer printer){
		...
		int numOfHorizontalPages = (int) (layerArea.width / (printArea.width / scaleFactor.x));
		int numOfVerticalPages = (int) (layerArea.height / (printArea.height / scaleFactor.y));
	...
	private class PrintJob implements Runnable {
			...
			if (printer.startJob("NatTable")) { //$NON-NLS-1$
				...
				for (int verticalPageNumber = 0; verticalPageNumber < pageCount.y; verticalPageNumber++) {
					...
					for (int horizontalPageNumber = 0; horizontalPageNumber < pageCount.x; horizontalPageNumber++) {
						...
						org.eclipse.swt.graphics.Rectangle printBounds = SWTUtil.toSWT(new Rectangle(
		 ...
		 */
		private void printLayer(GC gc, org.eclipse.swt.graphics.Rectangle printBounds) {
		 ...
		 * @param printBounds The bounds of the print page.
		 */
		private void printFooter(GC gc, int totalPageCount, org.eclipse.swt.graphics.Rectangle printBounds) {

...
public final class RangeList extends ArrayList<Range> implements Set<Range> {
	...
	public static RangeList toRangeList(final Collection<Range> collection) {
		if (collection instanceof RangeList) {
			return (RangeList) collection;
		}
		final RangeList list = new RangeList();
	 ...
	 */
	public RangeList(final long... initialValues) {
		this();
	...
	public final class Values implements /*OrderedSet<int>*/ Iterable<Long> {
		...
		public Range getRangeOf(final long value) {
			int idx = indexOfStart(value);
			if (idx >= 0) { // value == range1.start
	...
	public static Collection<Long> listValues(final Collection<Range> positions) {
